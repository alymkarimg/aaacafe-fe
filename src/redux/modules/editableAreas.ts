...
export interface IEditableAreaProps {
  guid: string;
  pathname: string;
  //other props
}

export interface IEditableAreaModels {
  content: string;
  guid: string;
  pathname: string;
  //other props
...
export interface StateProps {
  loading: number;
  error: string | null;
  editableAreaProps: IEditableAreaProps[];
  editableAreamodels: IEditableAreaModels[];
}
...
export const initialState: StateProps = {
  loading: 0,
  error: null,
  editableAreaProps: [{ guid: "1", pathname: "/" }],
  editableAreamodels: [],
};
...
export const loadDataAction =
  ...
  async (
    ...
    let data = initialState.editableAreaProps;
    ...
    try {
      await (async function load(): Promise<void> {
        ...
        const res = await axios.get<IEditableAreaModels[]>("/someurl");
        data = res.data;
      })();
    } catch (e) {
